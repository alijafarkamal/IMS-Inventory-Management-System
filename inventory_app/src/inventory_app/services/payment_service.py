...
def _select_gateway(method_type: str) -> object:
...
class PaymentService:
    ...
    def ensure_method(self, method_type: str, display_name: Optional[str] = None) -> PaymentMethod:
    ...
    def create_payment(self, order_id: int, method_type: str, amount: float, currency: str = "USD") -> Payment:
    ...
    def authorize_and_capture(self, payment_id: int, method_type: str) -> Payment:
    ...
    def refund(self, payment_id: int, method_type: str, amount: Optional[float] = None) -> Payment:
        gateway = _select_gateway(method_type)
        processor = PaymentProcessor(gateway=gateway, repo=self.repo)
